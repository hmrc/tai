...
class CacheConnectorItSpec
    extends AnyWordSpec with Matchers with GuiceOneAppPerSuite with MongoSupport with MongoFormatter with MockitoSugar
    with ScalaFutures with Injecting {
  ...
  private lazy val sut: CacheConnector = new CacheConnector(
    new TaiCacheRepository(mongoComponent, mockMongo, timestampSupport),
    new TaiCacheRepositoryUpdateIncome(mongoComponent, mockMongo, timestampSupport),
    mockMongo,
    configuration
  )
      ...
      "saved and returned json is valid" in {
        val data = sut
          .createOrUpdate[Person](
            cacheId,
            Person(nino, "Name", "Surname", None, Address("", "", "", "", ""), false, false))(
