...
abstract class BaseConnector(metrics: Metrics, httpClient: HttpClient)(implicit
  ec: ExecutionContext
  ...
  def getFromNps[A](url: String, api: APITypes, headerCarrier: Seq[(String, String)])(implicit
    hc: HeaderCarrier,
    formats: Format[A]
  ): Future[(A, Int)] = {
  ...
  @deprecated("this method will be removed. Use uk.gov.hmrc.tai.connectors.HttpHandler.getFromApi instead", "")
  def getFromDes[A](url: String, api: APITypes, headers: Seq[(String, String)])(implicit
    hc: HeaderCarrier,
    formats: Format[A]
  ): Future[(A, Int)] = {
    val timerContext = metrics.startTimer(api)
