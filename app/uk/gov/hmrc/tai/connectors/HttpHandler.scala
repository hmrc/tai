      ...
      override def read(method: String, url: String, response: HttpResponse): HttpResponse = {
        response.status match {
          ...
          case Status.NOT_FOUND => {
            Logger.warn(s"HttpHandler - No DATA Found error returned from $api with url $url")
            throw new NotFoundException(response.body)
          }
          case Status.INTERNAL_SERVER_ERROR => {
            Logger.warn(s"HttpHandler - Internal Server error returned from $api with url $url and message body ${response.body}")
            throw new InternalServerException(response.body)
          }
          case Status.BAD_REQUEST => {
            Logger.warn(s"HttpHandler - Bad request exception returned from $api with url $url")
            throw new BadRequestException(response.body)
          }
          case Status.LOCKED => {
            Logger.warn(s"HttpHandler - Locked response returned from $api with url $url")
            throw new LockedException(response.body)
          }
          case _ => {
            Logger.warn(s"HttpHandler - A Server error returned from $api with url $url and message body ${response.body}")
  ...
  def postToApi[I](url: String, data: I, api: APITypes)(implicit hc: HeaderCarrier, writes: Writes[I]): Future[HttpResponse] = {
    ...
    httpClient.POST[I, HttpResponse](url, data)(writes, rawHttpReads, hc, fromLoggingDetails) map { httpResponse =>
      httpResponse status match {
        ...
        case _ => {
          Logger.warn(s"HttpHandler - Error received with status: ${httpResponse.status}, url: $url and body: ${httpResponse.body}")
          metrics.incrementFailedCounter(api)
