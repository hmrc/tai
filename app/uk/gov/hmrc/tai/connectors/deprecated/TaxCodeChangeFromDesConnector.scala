...
import uk.gov.hmrc.tai.connectors.{BaseConnector, TaxCodeChangeFromDesUrl}
...
class TaxCodeChangeFromDesConnector @Inject()(
  metrics: Metrics,
  httpClient: HttpClient,
  config: DesConfig,
  taxCodeChangeUrl: TaxCodeChangeFromDesUrl)(
  ...
  def taxCodeHistory(nino: Nino, year: TaxYear)(implicit hc: HeaderCarrier): Future[TaxCodeHistory] = {
    val url = taxCodeChangeUrl.taxCodeChangeFromDesUrl(nino, year)
