...
case class EditableDetails(isEditable : Boolean = true,
...
case class TaxComponent(
                         amount : BigDecimal,
                         componentType : Int,
                         description : String,
                         iabdSummaries : List[IabdSummary])

object TaxComponent {
  implicit val formats = Json.format[TaxComponent]
}

case class TaxCodeIncomeSummary( name : String,
                                 taxCode : String,
                                 employmentId : Option[Int] = None,
...
object GateKeeper {
  ...
  def withMciRule: GateKeeper = GateKeeper(gateKeepered = true,
    List(GateKeeperRule(Some(TaiConstants.mciGateKeeperType),
