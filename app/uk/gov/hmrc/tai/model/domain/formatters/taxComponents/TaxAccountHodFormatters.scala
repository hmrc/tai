...
trait IncomeSourcesHodReads {
  ...
  private def codingComponentsFromJson(incomeJsVal: JsValue): Seq[CodingComponent] = {
    ...
    def allowanceFactory = (typeKey: Int, amount: BigDecimal, description: String, inputAmount: Option[BigDecimal]) =>
      npsComponentAllowanceMap.get(typeKey).map(CodingComponent(_, None, amount, description, inputAmount))

    def deductionFactory = (typeKey: Int, amount: BigDecimal, description: String, inputAmount: Option[BigDecimal]) =>
      npsComponentDeductionMap.get(typeKey).map(CodingComponent(_, None, amount, description, inputAmount))

    def nonTaxCodeIncomeFactory = (typeKey: Int, amount: BigDecimal, description: String, inputAmount: Option[BigDecimal]) =>
  ...
  private def codingComponentsFromIncomeSources(incomeSourceJson: JsValue, incomeSourceJsonElement: String,
  ...
  private def taxComponentFromNpsComponent(npsComponentJson: JsValue, codingComponentFactory: CodingComponentFactory) = {
...
trait TotalLiabilityHodReads extends BaseTaxAccountHodFormatters {
  ...
  private[taxComponents] val totalLiabilityReads = new Reads[Seq[CodingComponent]] {
    override def reads(json: JsValue): JsResult[Seq[CodingComponent]] = {
      ...
      val (benefits, otherCodingComponents) = codingComponents.partition{_.componentType match {
  ...
  private def reconcileBenefits(benefits: Seq[CodingComponent]): Seq[CodingComponent] = {
    val (benefitsFromEmployment, benefitsInKind) = benefits.partition(ben => benefitsFromEmploymentSet.contains(ben.componentType))
