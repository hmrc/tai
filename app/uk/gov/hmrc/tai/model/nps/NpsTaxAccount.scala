...
case class NpsTaxAccount(nino: Option[String],
  ...
  def toTaxSummary(version : Int, npsEmployments : List[NpsEmployment], iabds : List[NpsIabdRoot] = Nil,
                   rtiCalc : List[RtiCalc] = Nil, accounts: List[AnnualAccount] = Nil) : TaxSummaryDetails = {

    val childBenefitAmount = iabds.filter(_.`type` == ChildBenefit.code).foldLeft(BigDecimal(0))(_ + _.grossAmount.getOrElse(BigDecimal(0)))
    ...
    val taxSummaryDetails = TaxModelFactory.create(nino = nino.getOrElse(""), version,
    ...
    val hasDuplicateEmploymentNames = taxSummaryDetails.increasesTax.flatMap(_.incomes.map(_.taxCodeIncomes.hasDuplicateEmploymentNames)).getOrElse(false)

    val value = taxSummaryDetails.copy(incomeData = Some(getIncomeData(toTaxCodeIncomeTotal(filteredLive._1), toTaxCodeIncomeTotal( filteredLive._2),
      toTaxCodeIncomeTotal( filteredPensions._1),toTaxCodeIncomeTotal(filteredStateBenefits._1),rtiCalc,npsEmployments,iabds, hasDuplicateEmploymentNames)))
      value
  }
  ...
  def getIncomeData(employments: Option[TaxCodeIncomeTotal],
    ...
    def getIabdSummaries(emp: TaxCodeIncomeSummary) = {
      adjustedNetIncome.flatMap(_.iabdSummaries.map(
        _.filter(x => x.employmentId == emp.employmentId && x.`type`.contains(NewEstimatedPay.code))))
    }
    ...
    val incomeExplanations = allEmps.map { emp =>
      ...
      val payRollingBiks = npsEmp.exists(emp => emp.payrolledTaxYear.getOrElse(false) || emp.payrolledTaxYear1.getOrElse(false))
      val editableDetails = EditableDetails(payRollingBiks = payRollingBiks)
      val pay = iabds.find(iabd => iabd.`type` == NewEstimatedPay.code && iabd.employmentSequenceNumber == emp.employmentId)
      ...
      IncomeExplanation(employerName = emp.name,
  ...
  private[nps] def getTaxableStateBenefit(filteredIncomeSource : List[MergedEmployment]) : Option[NpsComponent] = {
    ...
    val iabdSummaries =
        getIadbSummary(filteredIncomeSource.filter(x => x.incomeSource.incomeType == IncomeTypeESA.code), EmploymentAndSupportAllowance.code) :::
        getIadbSummary(filteredIncomeSource.filter(x => x.incomeSource.incomeType == IncomeTypeJSA.code), JobSeekersAllowance.code) :::
        getIadbSummary(filteredIncomeSource.filter(x => x.incomeSource.incomeType == IncomeTypeIB.code), IncapacityBenefit.code)

    iabdSummaries match {
      case Nil => None
      case x => Some(NpsComponent(Some(x.foldLeft(BigDecimal(0))
        ((total,detail) => detail.amount.getOrElse(BigDecimal(0))  + total)), None, Some(x.sortBy(_.amount).reverse), None))
    }
  ...
  def getTaxCodeDetails(incomeSources: Option[List[NpsIncomeSource]] = None): Option[TaxCodeDetails] = {
    ...
    val taxCodeList = incomeSources.map(incomeSource => {
      ...
      val uniqueDeductions = deductions.groupBy(_.componentType).mapValues{y =>
      ...
      val uniqueAllowances: List[TaxCodeComponent] = allowances.groupBy(
        _.componentType
      ).values.toList.flatMap{y =>
        y.headOption.map(_.copy(
          amount = Some(y.flatMap(_.amount).sum))
        )
      }

      val codes = taxCodes.distinct.map(code =>
        TaxCode(Some(code), getTaxCodeRate(code))
      )
  ...
  def getTaxCodeComponent(npsComponents: Option[List[NpsComponent]]): Option[List[TaxCodeComponent]] = {
    npsComponents.map(tc => {
      tc.map(npsComponent => {
        TaxCodeComponent(description = npsComponent.npsDescription,
  ...
  private[nps] def getTaxCodeDescriptors(taxCode: String): List[String] = {
    ...
    val allPrefixMatches =  prefix.findAllMatchIn(taxCode).foldLeft(List[String]()){(list, m) => list :+ m.toString()}
    val filter = taxCode.filterNot( c => allPrefixMatches.contains(c.toString))
    val allSuffixMatches = suffix.findAllMatchIn(filter).foldLeft(List[String]()){(list, m) => list :+ m.toString()}
  ...
  private[nps] def getFullTaxCodeDescription(incomeSource: List[NpsIncomeSource]): List[TaxCodeDescription] = {
    incomeSource.foldLeft(List[TaxCodeDescription]()) { (col,incomes) =>
      ...
      val descriptors = getTaxCodeDescriptors(taxCode).map {
        d => TaxCode(Some(d), getTaxCodeRate(d))
      }
  ...
  private[nps] def getStatePensionLumpSum: Option[BigDecimal] = {
    incomeSources.flatMap (_.find(x => x.incomeType == IncomeTypeStatePensionLumpSum.code)).flatMap(
      _.payAndTax.flatMap(_.totalIncome).flatMap(_.amount))
  }
