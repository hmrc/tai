...
case class NpsAlreadyTaxedAtSource(
...
case class NpsBasicRateExtensions(
  amount: Option[BigDecimal] = None,
  `type`: Option[Int] = None,
  iabdSummaries: Option[List[NpsIabdSummary]] = None,
  npsDescription: Option[String] = None,
...
case class NpsReliefsGivingBackTax(
  amount: Option[BigDecimal] = None,
  `type`: Option[Int] = None,
  iabdSummaries: Option[List[NpsIabdSummary]] = None,
  npsDescription: Option[String] = None,
...
case class NpsTotalLiability(
  nonSavings: Option[NpsTax] = None,
  ...
  def getCodingAdjustment(
    relief: BigDecimal = BigDecimal(0),
    ...
    taxCodeDetails.map { code =>
      if (isAllow) {
        Adjustment(
          code.allowances
            .flatMap(allow => allow.find(_.componentType.contains(compType)))
            .flatMap(_.amount)
  ...
  def toTotalLiabilitySummary(
    underpaymentPreviousYear: BigDecimal,
    ...
    val reductions = LiabilityReductions(
      ...
      enterpriseInvestmentSchemeRelief = getCodingAdjustment(
        reliefsGivingBackTax
          .flatMap(_.enterpriseInvestmentSchemeRelief)
          .getOrElse(BigDecimal(0)),
        taxCodeDetails,
        AllowanceType.EnterpriseInvestmentSchemeRelief.id,
        isAllow = true
      ),
      concessionalRelief = getCodingAdjustment(
        reliefsGivingBackTax
          .flatMap(_.concessionalRelief)
          .getOrElse(BigDecimal(0)),
        taxCodeDetails,
        AllowanceType.ConcessionalRelief.id,
        isAllow = true),
      maintenancePayments = getCodingAdjustment(
        reliefsGivingBackTax
          .flatMap(_.maintenancePayments)
          .getOrElse(BigDecimal(0)),
        taxCodeDetails,
        AllowanceType.MaintenancePayment.id,
        isAllow = true),
      doubleTaxationRelief = getCodingAdjustment(
        reliefsGivingBackTax
          .flatMap(_.doubleTaxationRelief)
          .getOrElse(BigDecimal(0)),
        taxCodeDetails,
        AllowanceType.DoubleTaxationReliefAllowance.id,
        isAllow = true
      )
    )

    val additions = LiabilityAdditions(
      excessGiftAidTax = getCodingAdjustment(
        otherTaxDue.flatMap(_.excessGiftAidTax).getOrElse(BigDecimal(0)),
        taxCodeDetails,
        DeductionType.GiftAidAdjustment.id),
      excessWidowsAndOrphans = getCodingAdjustment(
        otherTaxDue.flatMap(_.excessWidowsAndOrphans).getOrElse(BigDecimal(0)),
        taxCodeDetails,
        DeductionType.WidowsAndOrphansAdjustment.id),
      pensionPaymentsAdjustment = getCodingAdjustment(
        otherTaxDue.flatMap(_.pensionPaymentsAdjustment).getOrElse(BigDecimal(0)),
        taxCodeDetails,
        DeductionType.Annuity.id)
    )
    ...
    val reductionsToLiability = liabilityReductions
      .flatMap(_.marriageAllowance.map(_.marriageAllowanceRelief))
      .getOrElse(BigDecimal(0)) +
      liabilityReductions
        .flatMap(_.enterpriseInvestmentSchemeRelief.map(_.amountInTermsOfTax))
        .getOrElse(BigDecimal(0)) +
    ...
    val additionsToTotalLiabilty = liabilityAdditions
      .flatMap(_.excessGiftAidTax.map(_.amountInTermsOfTax))
      .getOrElse(BigDecimal(0)) +
  ...
  def otherIncome(): Option[NpsComponent] = {
    ...
    val lossBroughtForward =
      allIADBTypes.find(_.`type`.contains(LossBroughtForwardFromEarlierTaxYear.code)).flatMap(_.amount)
  ...
  private def benefitsInKindRemovingTotalOrComponentParts(): List[NpsIabdSummary] = {
    ...
    val allBenefitsInKind: List[NpsIabdSummary] =
      allIADBTypes.filter(x => filterTypeAndZero(x, TaiConstants.IADB_TYPE_BENEFITS_IN_KIND))
    ...
    allEmploymentIds.flatMap { employmentId =>
      ...
      val calculatedTotalBenefitsInKind =
        benefitsInKind.foldLeft(BigDecimal(0))((total, detail) => detail.amount.getOrElse(BigDecimal(0)) + total)
