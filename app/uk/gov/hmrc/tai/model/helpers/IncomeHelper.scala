...
trait IncomeHelper {
  ...
  lazy val fetchIncomeType: Map[(Option[Int], Option[String]), String] = Map(
    ...
    (Some(TaiConstants.PENSION_FINANCIAL_ASSISTANCE_TAX_DISTRICT), Some(TaiConstants.PENSION_FINANCIAL_ASSISTANCE_PAYE_NUMBER)) -> IncomeTypeOccupationalPension.description,
  ...
  def isEditableByAutoUpdateService(otherIncomeSourceIndicator : Option[Boolean],
...
object IncomeHelper extends IncomeHelper {
  ...
  def getAllIncomes( npsEmployments : List[NpsEmployment],
    ...
    val mergedIncomes = incomeSources.map(_.map {
      incomeSource => {
        ...
        val estimatedPay = {
          val incomeTypeDescription = fetchIncomeType.get((incomeSource.employmentTaxDistrictNumber, incomeSource.employmentPayeRef))
          lazy val isJsaPrimary = incomeSource.employmentId.contains(TaiConstants.PrimaryEmployment) && incomeSource.jsaIndicator.getOrElse(false)

          (incomeTypeDescription, incomeSource.employmentType) match {
            case (Some(IncomeTypeESA.description), Some(TaiConstants.SecondaryEmployment)) => getESAFromAdjustedNetIncome(adjustedNetIncome)
            case (Some(IncomeTypeIB.description), Some(TaiConstants.SecondaryEmployment)) => getIBFromAdjustedNetIncome(adjustedNetIncome)
            case (Some(IncomeTypeJSA.description), Some(TaiConstants.SecondaryEmployment)) => getIBFromAdjustedNetIncome(adjustedNetIncome)
            case (None, Some(TaiConstants.SecondaryEmployment)) if isJsaPrimary => getIBFromAdjustedNetIncome(adjustedNetIncome)
    ...
    val mergedEmployments = npsEmployments.map {
      employment => {
        ...
        if (income.isEmpty) {
          ...
          val estimatedPay = {
            ...
            val incomeTypeDescription = fetchIncomeType.get((Some(employment.taxDistrictNumber.toInt), Some(employment.payeNumber)))
    ...
    def filterNonIncomeTypeStatePensionLumpSum(mergedEmployment: MergedEmployment) =
      !fetchIncomeType.get((mergedEmployment.incomeSource.employmentTaxDistrictNumber, mergedEmployment.incomeSource.employmentPayeRef)).contains(IncomeTypeStatePensionLumpSum.description)

    mergedIncomes.filter(mergedEmpl => filterNonIncomeTypeStatePensionLumpSum(mergedEmpl)) ::: mergedEmployments.filter(mergedEmpl => filterNonIncomeTypeStatePensionLumpSum(mergedEmpl))
  }

  private[helpers] def getEstimatedPayFromAdjustedNetIncome(adjustedNetIncome : Option[NpsComponent] = None, employmentId : Option[Int]) :Option[BigDecimal] = {
    val iabdFilter = (npsIabdSummary: NpsIabdSummary) => (npsIabdSummary.`type`.contains(NewEstimatedPay.code) && npsIabdSummary.employmentId == employmentId)
  ...
  private[helpers] def getESAFromAdjustedNetIncome(adjustedNetIncome: Option[NpsComponent] = None) = {
    val iabdFilter = (npsIabdSummary: NpsIabdSummary) => npsIabdSummary.`type`.contains(EmploymentAndSupportAllowance.code)
    getIabdAmountFromIncome(adjustedNetIncome, iabdFilter)
  }
  ...
  private[helpers] def getIabdAmountFromIncome(adjustedNetIncome : Option[NpsComponent], iabdFilter: NpsIabdSummary => Boolean) = {
    for {
  ...
  def filterTaxableStateBenefits(allIncomes: List[MergedEmployment]) :  (List[MergedEmployment], List[MergedEmployment]) = {
    allIncomes.partition(x => x.incomeSource.incomeType == IncomeTypeESA.code ||
  ...
  def isOccupationalPension(employmentTaxDistrictNumber: Option[Int],
  ...
  def incomeType(employmentTaxDistrictNumber: Option[Int],
                 employmentPayeRef : Option[String],
  ...
  def isPrimary(employmentType: Option[Int]): Boolean = employmentType.isEmpty || employmentType.contains(TaiConstants.PrimaryEmployment)

  def isEditableByUser(otherIncomeSourceIndicator: Option[Boolean],
  ...
  override def isEditableByAutoUpdateService(otherIncomeSourceIndicator: Option[Boolean],
                                    jsaIndicator: Option[Boolean]): Boolean = {
    !otherIncomeSourceIndicator.contains(true) && !jsaIndicator.contains(true)
  }

  private[helpers] def getFromAdjustedNetIncome(adjustedNetIncome : Option[NpsComponent] = None, iabdType: Int) :BigDecimal = {
