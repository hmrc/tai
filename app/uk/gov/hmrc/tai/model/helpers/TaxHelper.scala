...
trait TaxHelper {
  ...
  def toTax(oldTax : NpsTax, taxPaidAtSource : Option[BigDecimal] = None): Tax = {
    ...
    Tax(totalIncome = totalIncomeAmount,
      ...
      actualTaxDueAssumingBasicRateAlreadyPaid = taxCalculator.actualTaxDueAssumingBasicRateAlreadyPaid(totalIncomeAmount, oldTax.totalTax, filteredTaxBands),
      actualTaxDueAssumingAllAtBasicRate = taxPaidAtSource
    )
  }

  def toAdjustedTax(oldTax : NpsTax,
    ...
    val (adjustedTaxBands, adjustedTax, adjustedTaxable, adjustedAllowReliefDeducts) = taxCalculator.adjustTaxData(taxCode, allowances, deductions, oldTax)
    val filteredTaxBands = adjustedTaxBands.map(_.toList.sortBy(_.rate))

    val totalIncomeAmount = oldTax.totalIncome.flatMap(_.amount)

    Tax(totalIncome = totalIncomeAmount,
      totalTax = adjustedTax, totalTaxableIncome = adjustedTaxable,
      ...
      actualTaxDueAssumingBasicRateAlreadyPaid = taxCalculator.actualTaxDueAssumingBasicRateAlreadyPaid(totalIncomeAmount, adjustedTax, filteredTaxBands)
    )
  }
}
