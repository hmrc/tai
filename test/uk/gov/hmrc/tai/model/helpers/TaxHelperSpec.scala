  ...
  "Tax Helper" should {
    "return tax object" when {
      ...
      "given nps tax and tax paid at source" in {
        ...
        val oldTax =
          NpsTax(Some(NpsComponent(Some(1000))), Some(NpsComponent(Some(5000))), Some(6000), Some(200), Some(taxBands))
        val taxPaidAtSource = Some(BigDecimal(200))

        when(sut.taxCalculator.actualTaxDueAssumingBasicRateAlreadyPaid(any(), any(), any()))
          .thenReturn(Some(BigDecimal(1000)))
    ...
    "return adjusted tax" when {
      ...
      "given nps tax, tax code, allowances, deductions and potential underpayment" in {
        val sut = createSUT
        val taxBands = List(
          TaxBand(Some(1000), Some(0), None, Some(1500), Some(0)),
          TaxBand(Some(10000), Some(200), None, Some(11500), Some(20)))
        val oldTax =
          NpsTax(Some(NpsComponent(Some(100000))), Some(NpsComponent(Some(500))), Some(6000), Some(200), Some(taxBands))
        val taxCode = Some("BR")
        val potentialUnderPayment = Some(BigDecimal(100))

        when(sut.taxCalculator.adjustTaxData(any(), any(), any(), any()))
          .thenReturn((Some(taxBands), oldTax.totalTax, oldTax.totalTaxableIncome, Some(BigDecimal(100))))
        ...
        sut.toAdjustedTax(
          oldTax,
          taxCode,
          Some(List(NpsComponent(Some(1000)))),
          Some(List(NpsComponent(Some(5000)))),
          potentialUnderPayment) mustBe resTax
      }
    }
