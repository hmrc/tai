        ...
        Seq(
          IncomeSource(
            ...
            Employment(
              ...
              startDate = Some(LocalDate.parse(s"${TaxYear().next.year}-05-26")),
              endDate = None,
    ...
    "for current year" must {
      "return an income update success" when {
        "a valid update amount is provided" in {
          ...
          when(mockIabdService.updateTaxCodeAmount(any(), meq[TaxYear](taxYear), any(), any(), any())(any()))
            .thenReturn(
