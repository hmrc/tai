...
class EmploymentServiceSpec extends PlaySpec with MockitoSugar {
  ...
  "endEmployment" must {
    ...
    "Send end journey audit event for envelope id" in {
      ...
      val mockAuditable = mock[Auditor]
      doNothing()
        .when(mockAuditable).sendDataEvent(any(), any(), any(), any())(any())

      val sut = createSut(mockEmploymentRepository, mockPersonRepository, mock[IFormSubmissionService], mockFileUploadService, mockPdfService, mockAuditable)
      Await.result(sut.endEmployment(nino, 2, endEmployment), 5 seconds)

      verify(mockAuditable, times(1)).sendDataEvent(Matchers.eq("EndEmploymentRequest"), any(), any(),
  ...
  "addEmployment" must {
    ...
    "Send add journey audit event for envelope id" in {
      ...
      verify(mockAuditable, times(1)).sendDataEvent(Matchers.eq(IFormConstants.AddEmploymentAuditTxnName), any(), any(),
  ...
  "incorrectEmployment" must {
    ...
    "Send incorrect employment journey audit event" in {
      ...
      verify(mockAuditable, times(1)).sendDataEvent(Matchers.eq(IFormConstants.IncorrectEmploymentAuditTxnName), any(), any(),
        Matchers.eq(map))(any())
    }
  }
  ...
  "updatePreviousYearIncome" must {
    ...
    "Send incorrect employment journey audit event" in {
      ...
      verify(mockAuditable, times(1)).sendDataEvent(Matchers.eq(IFormConstants.UpdatePreviousYearIncomeAuditTxnName), any(), any(),
        Matchers.eq(map))(any())
    }
  }
