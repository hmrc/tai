...
import uk.gov.hmrc.time.TaxYearResolver.{startOfCurrentTaxYear, startOfNextTaxYear}
...
class TaiServiceSpec extends PlaySpec with MockitoSugar with NpsFormatter {
  ...
  "getCalculatedTaxAccountPartial" when {
    ...
    "desCall is true" should {
      ...
      when(mockDesConnector.getIabdsFromDes(any(), any())(any()))
      ...
      "return correct result" in {
        val taxSummary = npsTaxAccount.toTaxSummary(version, npsEmploymentList)
        result mustBe taxSummary
      }

      "call getEmployment" in {
        verify(mockNpsConnector, times(1))
          .getEmployments(Nino(nino.nino), taxYear)
      }
      "get iabds from des" in {
    ...
    "desCall is false" should {
      ...
      when(mockNpsConnector.getEmployments(Nino(nino.nino), taxYear))
        .thenReturn(Future.successful((npsEmploymentList, nps2EmploymentList, 1, Nil)))
      when(mockNpsConnector.getIabds(any(), any())(any()))
        .thenReturn(Future.successful(fakeIabds))
      ...
      "get iabds from nps" in {
        verify(mockNpsConnector, times(1))
          .getIabds(Nino(nino.nino), taxYear)
        verify(mockDesConnector, times(0))
          .getIabdsFromDes(Nino(nino.nino), taxYear)
      }
      "get taxAccount from nps" in {
        verify(mockNpsConnector, times(1))
          .getCalculatedTaxAccount(Nino(nino.nino), taxYear)
        verify(mockDesConnector, times(0))
          .getCalculatedTaxAccountFromDes(Nino(nino.nino), taxYear)
  ...
  "getAutoUpdateResults" should {
    ...
    when(mockAutoUpdatePayService.updateIncomes(any(), any(), any(), any(), any(), any())(any()))
      .thenReturn(rtiCalc)
  ...
  "isNotCeasedOrCurrentYearCeasedEmployment" should {
    ...
    when(mockTaxYearResolver.startOfCurrentTaxYear)
      .thenReturn(new LocalDate(taxYear, 4, 6))
    when(mockTaxYearResolver.startOfNextTaxYear)
      .thenReturn(new LocalDate(taxYear+1, 4, 6))
    ...
    "return false" when {
      ...
      "there is one employment with end date as start Of Current Tax Year" in {
        val endDate = Some(NpsDate(startOfCurrentTaxYear.withYear(taxYear)))
      ...
      "there is one employment with end date as start Of Next Tax Year" in {
        val endDate = Some(NpsDate(startOfNextTaxYear.withYear(taxYear + 1)))
        val employmentList = List(npsEmployment.copy(endDate = endDate))
        val result = Await.result(sut.isNotCeasedOrCurrentYearCeasedEmployment(employmentList), timeoutDuration)
        result mustBe false
      }
    ...
    "return true" when {
      ...
      "there is one employment with end date as one day after start Of Next Tax Year" in {
        val endDate = Some(NpsDate(startOfCurrentTaxYear.withYear(taxYear).plusDays(1)))
        val employmentList = List(npsEmployment.copy(endDate = endDate))
        val result = Await.result(sut.isNotCeasedOrCurrentYearCeasedEmployment(employmentList), timeoutDuration)
        result mustBe true
      }
      "there is one employment with end date as one day before start Of Next Tax Year" in {
        val endDate = Some(NpsDate(startOfNextTaxYear.withYear(taxYear + 1).minusDays(1)))
        val employmentList = List(npsEmployment.copy(endDate = endDate))
        val result = Await.result(sut.isNotCeasedOrCurrentYearCeasedEmployment(employmentList), timeoutDuration)
        result mustBe true
      }
  ...
  "appendCYPlusOneToTaxSummary" should {
    "return correct TaxSummaryDetails" in {
      ...
      val mockNextYearComparisonService = mock[NextYearComparisonService]
      when(mockNextYearComparisonService.stripCeasedFromNps(any()))
        .thenReturn(npsTaxAccount)
      when(mockNextYearComparisonService.proccessTaxSummaryWithCYPlusOne(any(), any()))
        .thenReturn(taxSummaryDetails)
  ...
  "invokeCYPlusOne" should {
    ...
    val mockYearResolver = mock[TaxYearResolver]
    when(mockYearResolver.taxYearFor(any()))
      .thenReturn(2017)
  ...
  private def createSUT(rti: RtiConnector,
    ...
    new TaiService(rti, nps, des, cid, autoUpdatePayService, nextYearComparisonService, Auditor,
      taxYearResolver, featureTogglesConfig, npsConfig, cyPlusOneConfig)
