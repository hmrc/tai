    ...
    "return the employment domain model" when {
      "there is no data in the cache" when {
        "the featureToggle to use RTI is set to false" in {
          ...
          val employment1 = npsDualEmployment._1
          val employment2 = npsDualEmployment._2

          val expectedEmployment1 = employment1.copy(annualAccounts = Seq(annualAccount1))
          val expectedEmployment2 = employment2.copy(annualAccounts = Seq(annualAccount2))

          val expectedEmployments = Seq(expectedEmployment1, expectedEmployment2)
          ...
          when(
            mockEmploymentBuilder.combineAccountsWithEmployments(
              Matchers.eq(Seq(employment1, employment2)),
          ...
          when(
            mockEmploymentBuilder
              .combineAccountsWithEmployments(
                ...
                Matchers.eq(nino),
                Matchers.eq(TaxYear(2017)))(any()))
            .thenReturn(Seq(expectedEmployment))
...
//  }
  ...
  //TODO are these required outside or can they be in lined
  val mockRtiConnector = mock[RtiConnector]
  val mockCacheConnector = mock[CacheConnector]
  val mockNpsConnector = mock[NpsConnector]
  ...
  private def testController(
    ...
    employmentBuilder: EmploymentBuilder = mockEmploymentBuilder,
    ...
    new EmploymentRepository(rtiConnector, cacheConnector, npsConnector, auditor, employmentBuilder, featureToggle)
  }
