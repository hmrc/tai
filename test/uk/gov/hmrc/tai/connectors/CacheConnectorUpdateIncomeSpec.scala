...
class CacheConnectorUpdateIncomeSpec extends BaseSpec with MongoFormatter with IntegrationPatience {
  ...
  private def createSUT(mongoConfig: MongoConfig = mock[MongoConfig]) =
    new CacheConnectorUpdateIncome(taiUpdateIncomeRepository, mongoConfig, configuration)
  ...
  "CacheConnectorUpdateIncome" must {
