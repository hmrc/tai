...
class CachingRtiConnectorSpec extends ConnectorBaseSpec {
  ...
  override implicit lazy val app: Application = GuiceApplicationBuilder().overrides(
  ...
  override def beforeEach(): Unit = reset(mockRtiConnector, mockSessionCacheRepository)
  ...
  val payment = Payment(LocalDate.now(), 0, 0, 0, 0, 0, 0, FourWeekly, None)
  val annualAccount = AnnualAccount(0, TaxYear(2020), Available, Seq(payment), Seq.empty)
