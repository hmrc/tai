...
class BaseConnectorSpec extends PlaySpec with MockitoSugar {
  ...
  "BaseConnector" should {
    ...
    "add extra headers for NPS" in {
      val headers = createSUT(mock[Auditor], mock[Metrics], mock[HttpClient]).extraNpsHeaders(HeaderCarrier(),23,"testtxID").headers
    ...
    "start and stop a transaction timer" when {
      ...
      "making a GET request to Citizen Details" in {
        ...
        val fakePersonalDetails = PersonDetails("4", Person(Some("Monkey"), None, None, None, Some("King"), Some("Great Sage equal of heaven"), None, None, Nino(nino.nino), Some(true), Some(false)))
    ...
    "return a success response from citizen details" when {
      "it returns a success Http response for GET transactions" in {
        ...
        val fakePersonalDetails = PersonDetails("4", Person(Some("TestName"), None, None, None, Some("TestTitle"), Some("TestHonours"), None, None, Nino(nino.nino), Some(true), Some(false)))
